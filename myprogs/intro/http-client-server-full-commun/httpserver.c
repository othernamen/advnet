...
char request_t[32]="";
	...
	for (n=0; request[n]; request[n]==' ' ? n++ : *(request++));			// Count how many columns in request (count spaces)
	if(n==2)								// If there are 3 columns (2 spaces) 
		if (string != NULL) {
 	       	token = strsep(&string, " ");
       		strcpy(request_t, token);					// Copy first column (GET) to request_t
	        token = strsep(&string, " ");					
       		token = strsep(&string, " ");
	        strncpy(http_version, token, 5);
		if(strstr("GET",request_t) && strstr("HTTP/", http_version))
